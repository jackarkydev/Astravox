...
```mermaid
flowchart LR
    PROG[PROGRAM] --> ANALYSIS[Analysis Filter Bank]
    ANALYSIS --> ENV[Envelope Followers]
    CARRIER[CARRIER / VCO / Noise] --> SYNTH[Synthesis Filter Bank]
    ENV --> VCA[Band VCAs]
    SYNTH --> VCA
    VCA --> MIX[Mix]
    MIX --> OUTVCA[VCA]
...
- 24 Bark bands, extending the core's 16
- 8 additional fader strips
- Per-band CV I/O and freeze gates across all 24 bands
